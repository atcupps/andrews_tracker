...
print("Sending request to Supabase for course list file download.")
response = supabase.storage.from_(bucket_name).download(file_path).decode()
...
while l < length:
    ...
    for course_id in codes_for_url:
        print(f"Parsing seats info for: {course_id}", end='\r')
